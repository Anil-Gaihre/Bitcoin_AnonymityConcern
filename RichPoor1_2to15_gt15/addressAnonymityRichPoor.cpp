...
int main(int args, char **argv)
{
    ...
    string pathtoAnonymityconcernedaddresses=path+"//AnonymityConcerned-Date-NRichAddressto0-NPaddressesto0-Indeg_1-Indeg_2_15-Inde_g15.dat";
        ...
        {            
            ...
            if (ginst->addressMap[j]=='I')//Condition where the address is the Input to the transaction
            {
                ...
                if(VerticesMoney[ginst->csr[j]]==0)
                {
                    ...
                    else if ((indegree[ginst->csr[j]]>1)&& (indegree[ginst->csr[j]]<16))
                    {
